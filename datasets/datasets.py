...
def _extract_riva_plate_id(filename_stem: str) -> str:
    """
    Extract plate number from RIVA crop stem.
    Example stem: HSIL_25_9_A2_00019_CA -> plate_id = "25"
    """
    # first token can vary; second token is the plate id
    m = re.match(r"^[^_]+_([^_]+)_", filename_stem)
    if not m:
        raise ValueError(
            f"Could not parse plate id from RIVA filename stem: {filename_stem!r}"
        )
    return m.group(1)
...
def scan_riva(
    ...
    *,
    ...
    """
    Scan RIVA and return a DataFrame compatible with this repository:
      - path
      - label_full
      - binary_label ("normal"/"abnormal")
      - binary_idx (0/1)
      - cluster_id (plate-based group)
      - split ("train_dev"/"test")
      - fold (0..num_folds-1 inside train_dev, -1 for test)

    Notes
    -----
    - Only these classes are used:
        abnormal: {"CA", "HSIL"}
        normal:   {"Sin_lesion", "ENDO"}
      Other RIVA folders are ignored for this binary setup.
    - Train/test split is group-aware by plate id (cluster_id) to prevent leakage.
    - Fold split inside train_dev is stratified by binary class, not group-aware.
    """
    records = []
    ...
    for cls_dir in sorted(p for p in root.iterdir() if p.is_dir()):
        label = cls_dir.name
        if label not in RIVA_TARGET_CLASSES:
            skipped_classes.append(label)
            continue

        for img_path in sorted(cls_dir.glob("*.png")):
            plate_id = _extract_riva_plate_id(img_path.stem)
            cluster_id = f"plate_{plate_id}"
            records.append((str(img_path), label, cluster_id))

    if not records:
        raise RuntimeError(
            f"No usable RIVA PNG images found under {root}. "
            f"Expected folders from {sorted(RIVA_TARGET_CLASSES)}."
        )

    if skipped_classes:
        print(
            f"[scan_riva] Ignoring non-binary folders: {sorted(set(skipped_classes))}"
        )

    df = pd.DataFrame(records, columns=["path", "label_full", "cluster_id"])
    df["binary_label"] = df["label_full"].map(
        lambda x: "normal" if x in RIVA_NORMAL else "abnormal"
    )
    df["binary_idx"] = df["binary_label"].map({"normal": 0, "abnormal": 1})

    if df["binary_idx"].isna().any():
        bad = sorted(df[df["binary_idx"].isna()]["label_full"].unique().tolist())
        raise ValueError(f"Unexpected RIVA labels after mapping: {bad}")

    # Group-aware train/test split by plate id
    cluster_labels = df.groupby("cluster_id")["binary_idx"].max()
    cluster_ids = cluster_labels.index.to_numpy()
    cluster_y = cluster_labels.to_numpy()
    ...
    if overlap:
        raise RuntimeError(f"RIVA cluster leakage detected: {sorted(overlap)}")

    df["split"] = "train_dev"
    df.loc[df["cluster_id"].isin(test_clusters), "split"] = "test"

    # CV folds inside train_dev (sample-level stratification)
    df["fold"] = -1
    train_dev_mask = df["split"] == "train_dev"
    train_dev_idx = df.index[train_dev_mask].to_numpy()
    train_dev_labels = df.loc[train_dev_mask, "binary_idx"].to_numpy()
    ...
    skf = StratifiedKFold(n_splits=num_folds, shuffle=True, random_state=seed)
    for fold_idx, (_, val_rel_idx) in enumerate(
        skf.split(train_dev_idx, train_dev_labels)
    ):
        val_idx = train_dev_idx[val_rel_idx]
        df.loc[val_idx, "fold"] = fold_idx
...
# NOTE:
# Keep this module import-safe. Do not perform dataset scans or transform
# creation at import time; callers should invoke scan_* / make_tf_* explicitly.
